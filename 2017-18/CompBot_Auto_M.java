...
@Autonomous(name = "CompBot Auto Master", group = "Linear Opmode")
//@Disabled

public class CompBot_Auto_M extends LinearOpMode{ //master as of 1/4/18
    ...
    public void runOpMode() {
        ...
        while(opModeIsActive()) {
            ...
            telemetry.addData("L Color (green):", leftSensor.green());
            telemetry.update();

            switch(step) {
                case 1: mark = Functions.readCipher(telemetry, vuforia); //read the cipher
                    if (mark.equals(""))
                        mark = "MIDDLE"; //default value in case the reading doesn't go well
                    step = 2;
                    break;
                case 2: rightKnock.setPosition(Servo.MAX_POSITION); //hopefully this is down. Also, use leftKnock.setPosition(Servo.MIN_POSITION) in appropriate circumstance.
                    double power = 0;
                    if(Functions.checkColor(rightSensor, "BLUE")) //use "RED" as necessary
                        power = 0.5;
                    else
                        power = -0.5;
                    t.reset();
                    while(t.time() < 2)
                        rightDrive.setPower(power); //use leftDrive as needed
                    rightDrive.setPower(0);
                ...
                case 4: double ti = 0; //go to the appropriate column - change values for other side
                    switch(mark) {
                        ...
                        case "RIGHT": ti = 1; break; //Assuming that "RIGHT" is the ROBOT'S "RIGHT"!!!
                    }
